package com.htkapp.core.shiro.core.shiro.session;


import com.htkapp.core.shiro.common.utils.LoggerUtils;
import com.htkapp.core.shiro.common.utils.StringUtils;
import com.htkapp.core.shiro.core.shiro.CustomShiroSessionDAO;
import com.htkapp.core.shiro.core.shiro.UserOnlineBo;
...
public class CustomSessionManager {
	 ...
	 * session status 
	 */
	public static final String SESSION_STATUS ="htk-online-status";
	@Resource
	...
	private UserOnlineBo getSessionBo(Session session){
		...
		if(obj instanceof SimplePrincipalCollection){
			...
			if(null != obj && obj instanceof LoginUser){
				//存储session + user 综合信息
				UserOnlineBo userBo = new UserOnlineBo((LoginUser)obj);
				//最后一次和系统交互的时间
	...
	public void forbidUserById(Long id, Long status) {
		...
		for(UserOnlineBo bo : getAllUser()){
			Long userId = bo.getUserId().longValue();
			//匹配用户ID
			if(userId.equals(id)){
				//获取用户Session
